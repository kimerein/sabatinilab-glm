...
def to_profile():
    ...
    # prefix = 'all_data_v12-61-63-64-together2'
    prefix = 'all_data_v12-61-63-64-words'
    ...
    raw_file_loc = '/Users/josh/Documents/Harvard/GLM/sabatinilab-glm/sglm/data/raw'
    ...
    files_list = glob.glob(f'{raw_file_loc}/GLM_SIGNALS_WT61_*') + \
                 glob.glob(f'{raw_file_loc}/GLM_SIGNALS_WT63_*') + \
                 glob.glob(f'{raw_file_loc}/GLM_SIGNALS_WT64_*')
    ...
    for ifn,filename in enumerate((files_list)):
        ...
        # Load file
        df = pd.read_csv(f'{raw_file_loc}/{filename}')
        ...
        table_fn = f'{raw_file_loc}/{filename}'.replace('GLM_SIGNALS', 'GLM_TABLE')
        # print(fn, '--', table_fn)
    ...
    df['spnrOff'] = ((df['spnr'] == 1)&(df['photometrySideInIndex'] != 1)).astype(int)
    df['spnnrOff'] = ((df['spnnr'] == 1)&(df['photometrySideInIndex'] != 1)).astype(int)
    df['spxrOff'] = ((df['spxr'] == 1)&(df['photometrySideOutIndex'] != 1)).astype(int)
    df['spxnrOff'] = ((df['spxnr'] == 1)&(df['photometrySideOutIndex'] != 1)).astype(int)
    # print(df[['spnr', 'spnnr', 'spxr', 'spxnr']].sum())
    print(df[X_cols_all].sum())
    ...
    # for y_col in tqdm(['zsrdFF', 'zsgdFF'], 'ycol'):
        ...
        for left_out in (leave_one_out_list):
            ...
            watch = (dfrel[drop_cols_basis + X_cols_sftd + [y_col]].isna()*1).idxmax(axis=1)
